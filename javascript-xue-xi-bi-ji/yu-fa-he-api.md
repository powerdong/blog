...
2. 全局变量会挂载在 window 对象下,会成为 window 下的一个属性。
3. 如果你没有使用严格模式并给一个未声明的变量赋值的话,JS 会自动创建一个全局变量。
...
| <pre><code>1</code></pre> | <pre><code>array.map(function(ele, index, arr) {}, thisValue); //浅克隆</code></pre> |
| ------------------------- | --------------------------------------------------------------------------------- |
...
| <pre><code>123456789101112131415161718192021222324</code></pre> | <pre><code>/** * 深度克隆 * @param {*} Target 目标对象 * @param {*} Option 克隆源 */function deepClone(Target, Option) {  if (Option != null) {    for (var prop in Option) {      var src = Target[prop];      var copy = Option[prop];      if (copy &#x26;&#x26; typeof copy == "object") {        if (Object.prototype.toString.call(copy) == "[object Array]") {          src = [];        } else {          src = {};        }        Target[prop] = deepClone(src, copy);      } else {        Target[prop] = copy;      }    }  }  return Target;}</code></pre> |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |

1. **reduce**

| <pre><code>1234</code></pre> | <pre><code>array.reduce(function(preValue, ele, index, arr), initialValue)//initialValue :传初始值;// 如果不传,第一次pre打印数值第一位,ele是第二位</code></pre> |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
...
| <pre><code>12345678910111213141516</code></pre> | <pre><code>/***底层实现*/Array.prototype.Reduce = function(func,init){    var previous = init,        k = 0;    if(init === undefined){        previous = this[0];        k = 1;     }    for(k; k&#x3C;this.length; k++){        previous = func(previous, this[k], k);    }    return previous;}</code></pre> |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

1. **filter**

| <pre><code>1</code></pre> | <pre><code>array.filter(function(ele,index,arr){}, thisValue)</code></pre> |
| ------------------------- | -------------------------------------------------------------------------- |
...
| <pre><code>12345678910111213</code></pre> | <pre><code>/***底层实现*/Array.prototype.Filter = function(func){    var arr = [];    for(var i = 0; i&#x3C;this.length; i++){        if(func(this[i], i){            arr.push(this[i]);        };    }    return arr;}</code></pre> |
| ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

1. **forEach**

| <pre><code>1</code></pre> | <pre><code>array.forEach(function(ele,index,arr){}, thisValue)</code></pre> |
| ------------------------- | --------------------------------------------------------------------------- |
